...
namespace EcommerceApp.Services.Tests
{
    ...
    public class ReviewServiceTests
    {
        ...
        [Test]
        public async Task GetAllReviewsByProductIdAsync_ShouldReturnEmptyList_WhenNoReviewsMatch()
        {
            int productIdWithNoReviews = 99;
            ...
            var mockReviewsQueryable = allReviewsData
                .AsQueryable()
                .BuildMock();
            ...
            var result = await this.reviewService
                .GetReviewsByProductIdAsync(productIdWithNoReviews);
        ...
        [Test]
        public async Task GetAllReviewsByProductIdAsync_ShouldReturnMappedReviews_WhenReviewsMatch()
        {
            ...
            var mockReviewsQueryable = reviewsData
                .AsQueryable()
                .BuildMock();
            ...
            var result = (await this.reviewService
                .GetReviewsByProductIdAsync(targetProductId)).ToList();
        ...
        public async Task GetReviewByIdAsync_ShouldReturnNull_WhenReviewNotFound()
        {
            ...
            var result = await this.reviewService
                .GetReviewByIdAsync(nonExistentReviewId);
        ...
        public async Task GetReviewByIdAsync_ShouldReturnReview_WhenReviewExists()
        {
            ...
            var listWithReview = new List<Review> { reviewData }
                .AsQueryable()
                .BuildMock();
        ...
        public async Task AddReviewAsync_ValidInput_ShouldAddAndSaveChanges()
        {
            ...
            var userForDb = new ApplicationUser 
            { 
                Id = userId 
            };

            var destinationForDb = new Product 
            { 
                Id = model.ProductId 
            };
            ...
            this.repository
                ...
                .Returns(new List<Product> { destinationForDb }
                .AsQueryable()
                .BuildMock());

            this.userManager
                .Setup(um => um.FindByIdAsync(userId))
                .ReturnsAsync(userForDb);

            this.htmlSanitizer
                .Setup(s => s.Sanitize(model.Comment, It.IsAny<string>(), It.IsAny<IMarkupFormatter>()))
            ...
            Assert.AreEqual(model.ProductId, capturedReview.ProductId);
        }

        [Test]
        public void AddReviewAsync_ProductNotFound_ShouldThrowInvalidOperationException()
        {
            var model = new AddReviewViewModel 
            { 
                ProductId = 99, 
            ...
            this.repository.Setup(r => r.AllReadonly<Product>())
            ...
            Assert.AreEqual("The product does not exist.", ex.Message);
        }
        ...
        public async Task UpdateReviewAsync_UserIsOwner_ShouldUpdateAndSaveChanges()
        {
            ...
            this.repository
                .Setup(r => r.All<Review>())
                .Returns(new List<Review> { existingReview }
                .AsQueryable()
                .BuildMock());
        ...
        public void UpdateReviewAsync_UserNotOwnerNorAdmin_ShouldThrowInvalidOperationException()
        {
            ...
            this.userManager
                .Setup(um => um.FindByIdAsync(attackerUserId))
                .ReturnsAsync(attackerUser);

            this.userManager
                .Setup(um => um.IsInRoleAsync(attackerUser, AdminRoleName))
                .ReturnsAsync(false);

            var ex = Assert.ThrowsAsync<UnauthorizedAccessException>(() =>
            this.reviewService.UpdateReviewAsync(model, attackerUserId));

            Assert.AreEqual("You do not have permission to edit this review.", ex.Message);
        }
        ...
        public async Task DeleteReviewAsync_UserIsAdmin_ShouldDeleteAndSaveChanges()
        {
            ...
            this.userManager
                .Setup(um => um.FindByIdAsync(adminUserId))
                .ReturnsAsync(adminAppUser);

            this.userManager
                .Setup(um => um.IsInRoleAsync(adminAppUser, AdminRoleName))
                .ReturnsAsync(true);

            this.repository
                .Setup(r => r.Delete(reviewToDelete));
        ...
        public void DeleteReviewAsync_UserNotOwnerNorAdmin_ShouldThrowUnauthorizedAccessException()
        {
            ...
            Assert.AreEqual("You do not have permission to delete this review.", ex.Message);
        }
    }
}